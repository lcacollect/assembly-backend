...
@pytest.mark.asyncio
async def test_update_project_assembly_layers(client: AsyncClient, assembly_with_layers, project_epds):
...
@pytest.mark.asyncio
async def test_delete_project_assembly_layers(client: AsyncClient, assembly_with_layers, project_epds, db):
    assembly = assembly_with_layers
    mutation = """
    ...
    async with AsyncSession(db) as session:
        query = select(ProjectAssembly).where(ProjectAssembly.id == assembly.id).options(selectinload(ProjectAssembly.layers))
...
@pytest.mark.asyncio
async def test_add_project_assembly_layers_with_transport(client: AsyncClient, assemblies, project_epds):
    assembly = assemblies[0]
